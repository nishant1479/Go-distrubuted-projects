...
func ListenGRPC(s Service,accountURL,catalogURL string,port int)error {
	...
	catalogClient, err := catalogURL.NewClient(catalogURL)
	if err != nil{
		accountClient.Close()
		return err
	}
	...
	serv := grpc.NewServer()
	pb.RegisterOrderService(serv,&grpcServer{
		s,
		accountClient,
		catalogClient,
	})
...
func(s *grpcServer) PostOrder(ctx context.Context,r *pb.PostOrderRequest)(*pb.PostOrderResponse,error){
	...
	productIDs := []string{}
	orderedProduct,err := s.catalogClient.GetProducts(ctx,0,0,productIDs,"")
	if err != nil {
		log.Println("Error gettting products: ",err)
